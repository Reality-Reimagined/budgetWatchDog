    ...
    async function fetchData() {
      try {
        const [gdpResponse, inflationResponse, employmentResponse] = await Promise.all([
          fetch('http://localhost:8000/api/data?metric=gdp_growth'),
          fetch('http://localhost:8000/api/data?metric=inflation_rate'),
          fetch('http://localhost:8000/api/data?metric=employment_growth'),
        ]);
