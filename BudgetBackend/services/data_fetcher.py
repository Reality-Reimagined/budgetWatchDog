...
# API Endpoints for Provinces and Federal Data
PROVINCE_API_BASES = {
...
class DataFetcher:
    ...
    def fetch_data(government_level: str, province: str, dataset: str, params: dict = None):
        ...
        if response.status_code == 200:
            ...
            logging.error(f"Failed to fetch data from {province}: {response.status_code}")
    ...
    def fetch_economic_data(metric: str):
        try:
            ...
            elif metric == "bond_yields":
                url = f"{BANK_OF_CANADA_API}/observations/FRB"
                response = requests.get(url, params={"format": "json"})
            ...
            if response.status_code == 200:
                ...
                logging.error(f"Failed to fetch {metric}: {response.status_code}")
                raise Exception(f"Failed to fetch {metric}: {response.status_code}")
        
        except Exception as e:
            logging.error(f"Error fetching economic data for {metric}: {str(e)}")
            raise
